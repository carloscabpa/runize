...
class RevSliderTemplate {
	...
	public $omited_templates = array(
            'clean-news-post-based',
            'standard-wp-gallery',
            'woocommercesmall',
            'woobig',
            'news-gallery-post-based', 
        );
			...
			if(empty($cur)){
				...
				if(isset($new['slider']) && is_array($new['slider'])){
					...
					$cur['slides'] = $new['slides']; // push always all slides
                        ...
                        $omited_templates = $this->omited_templates;
                       //    var_dump($cur);die();
                         $count_slider = 0;
                         foreach($cur['slider'] as $template_slider){
                            if(in_array($template_slider['alias'],$omited_templates)){
                         ...
                         foreach($cur['slides'] as $template_slides){
                            if(in_array($template_slides,$omited_templates)){
		...
		if(!empty($sliders)){
			foreach($sliders as $slider){
				$slides = $this->getThemePunchTemplateDefaultSlides($slider['alias']);
                             //   var_dump($wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", $slider['id'])));die();
				//$test = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", $slider['id']));
                              //  var_dump($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", $slider['id']));die('yes');
				$slider_result = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %s", $slider['id']), ARRAY_A);
                                if(!isset($slider['installed']) && is_array($slider_result)){
					$templates = array_merge($templates, $slider_result);
				}else{
