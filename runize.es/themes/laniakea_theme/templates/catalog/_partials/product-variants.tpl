 ...
 *}
  {foreach from=$groups key=id_attribute_group item=group name=groups}
    ...
    <div class="product-variants-item d-flex align-items-center">
      <div class="control-label text-uppercase">{$group.name} : </div>
        ...
        <ul id="group_{$id_attribute_group}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="pull-xs-left input-container">
              ...
              <span
                {if $group_attribute.html_color_code}class="color{if $group_attribute.html_color_code == '#ffffff'} white{/if}{if $group_attribute.selected} active{/if}" style="background-color: {$group_attribute.html_color_code}"{/if}
