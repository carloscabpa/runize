    ...
    {foreach from=$facets item="facet"}
      {if $facet.displayed}
        <section class="facet{if $facet.label == "Size"} facet-size{/if}">
          <div class="facet-title hidden-sm-down">{if $facet.label == 'Brand'}{l s='Manufacture'}{else}{$facet.label}{/if}</div>
          ...
          {if $facet.widgetType !== 'dropdown'}
            <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if}">
              {foreach from=$facet.filters item="filter"}
                {if $filter.displayed}
                  <li{if isset($filter.properties.color)} class="facet_color"{/if}{if $facet.label == "Size"} class="facet_size"{/if}>
                      ...
                      {if $facet.multipleSelectionAllowed}
                        <span class="custom-checkbox">
                          ...
                          {if isset($filter.properties.color)}
                            <span class="color{if $filter.label =='White'} white{/if}{if $filter.active } checked {/if}" style="background-color:{$filter.properties.color}"><i class="material-icons checkbox-checked">check</i></span>
