        ...
        <div class="d-flex align-items-center mb-15 pb-5">
          ...
          <div class="ml-auto d-flex align-items-center">
            ...
            {if Configuration::get('smartenablecomment') == 1}
              {if $comment_status == 1}
                <div class="reply m-0">
                  <a onclick="return addComment.moveForm('comment-{$comment.id_smart_blog_comment}', '{$comment.id_smart_blog_comment}', 'respond', '{$id_post}')" class="comment-reply-link">{l s="Reply" d='Modules.Smartblog.Comment_loop'}</a>
                </div>
              {/if}
            {/if}
          </div>
