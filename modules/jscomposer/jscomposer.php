...
class JsComposer extends Module implements WidgetInterface
{
    ...
    public static $backoffice_inline_styles = array(),
    $backoffice_styles                      = array(),
    $backoffice_scripts                     = array(),
    $google_front_css                       = array(),
    $front_styles                           = array(),
    $front_scripts                          = array(),
    $front_inline_tags                      = array();
    public $admin_media_path_css;
    public $admin_media_path_js;
    public $asset_media_path, $_url_ultimate,
    $googleFontsEditClass;
    // ultimate------
    ...
    { 
        ...
        if($controller == 'cms'){
            if (isset($this->context->smarty->tpl_vars['cms']->value['id'])) {
                ...
                $this->init(); 
                $content = self::do_shortcode( $content ); 
        ...
        if($controller == 'product'){
            if (isset($this->context->smarty->tpl_vars['product']->value['id'])) {
                ...
                $this->init();  
                $descriptionChange = self::do_shortcode( $product_var->value['description'] ); 
        ...
        if($controller == 'category'){
            if (isset($this->context->smarty->tpl_vars['category']->value['id'])) {
                ...
                $this->init();  
                $descriptionChange = self::do_shortcode( $category_var->value['description'] ); 
...
if (isset($this->context->smarty->tpl_vars['manufacturer']->value['id'])) {
    ...
    $this->init();  
    $descriptionChange = self::do_shortcode( $manufacturer_var->value['description'] ); 
...
// below all is for supplier single page---------------------------------------------
        ...
        if (isset($this->context->smarty->tpl_vars['supplier']->value['id'])) {
            ...
            $this->init();  
            $descriptionChange = self::do_shortcode( $supplier_var->value['description'] ); 
...
// below all is for supplier list page page---------------------------------------------
        ...
        if (isset($this->context->smarty->tpl_vars['brands']->value)) {
            ...
            foreach($supplier_list->value as $supplier){ 
                ...
                $this->init();  
                $descriptionChange = self::do_shortcode( $supplier_var['description'] ); 
        ...
        if (!self::$isVcAdminCustomController) {
            ...
            if ( Tools::version_compare( _PS_VERSION_, '1.7.6.0', '>=' ) ) {
                if ( Tools::getValue( 'controller' ) == 'AdminBlogPost' ) {
                    ...
                    $content .= 'var page_id = ' . self::getAdminId() . ";\n";
                    ...
                    $content .= 'var page_id = ' . ( Tools::getValue( 'val_identifier' ) ? Tools::getValue( 'val_identifier' ) : 'null' ) . ";\n";
                }
                 else {
...
//        var_dump($_GET);die();
        foreach($_GET as $key => $value){
            ...
            if(((($ck_process_type_add == 'add' || $ck_process_type_update == 'update') && ($value == '' || $value == '1')) || ($id_product && $controller == 'AdminProducts') || ($controller == 'AdminCmsContent') || ($controller == 'AdminCategories') || ($controller == 'AdminManufacturers')) && is_object($modules_configuration)){
...
//var_dump($modules_configuration);die();
     ...
     public static function isNotFormPage(){ 

        $controller = Tools::getValue('controller');
        // var_dump($controller);die();
        if( $controller == 'AdminCmsContent' || $controller == 'AdminCategories'  || $controller == 'AdminManufacturers' ){
            //do nothing and carry on the function
        }else{
            return false;
        }
        ...
        // return false;
         ...
         $haystack = "$actual_link";
         $needle1   = "/edit";
         $needle2   = "/new";


         // separate check for manufacturer address page
    ...
    public function loadVcFrontendActionScripts()
    {
        ...
        if (Tools::version_compare(_PS_VERSION_, '1.7.5.0', '>')){ 
           $this->addCSS(__PS_BASE_URI__ . $this->context->controller->admin_webpath . '/themes/default/public/theme.css'); 
           //  echo "<style>#nav-sidebar{display:none;}</style>";
        }
...
//        print_r($_POST);die();
    ...
    public function hookvcBeforeInit() {
        ...
        $this->DoAction( 'mapcode' );
        if ( $this->is_admin() ) {
            $this->context->controller->addJquery();
            $this->smart_vc_map();
            if ( Tools::getvalue( 'controller' ) == 'VC_frontend' ) {
                $this->DoAction( 'addcode' );
            }
        } else {
            $this->DoAction( 'addcode' );
        }
    ...
    public function hookDisplayHeader( $params ) {
        ...
        // $id_cms = $this->context->smarty->tpl_vars['cms']->value['id'];
        // $content = &$this->context->smarty->tpl_vars['cms']->value['content'];
        // var_dump(self::do_shortcode($content));die();
        // $content=self::do_shortcode($content);
        //
        // }

        // ultimate--------------------------------
        $this->context->controller->addCSS( $this->asset_media_path . 'min-css/override.theme.css' );
        $this->context->controller->addCSS( $this->asset_media_path . 'min-css/animate.min.css' );
        $this->context->controller->addCSS( $this->asset_media_path . 'min-css/style.min.css' );
        $this->context->controller->addCSS( $this->asset_media_path . 'min-css/vc-fronteditor.min.css' );
        if ( ! empty( self::$front_styles ) ) {
            foreach ( self::$front_styles as $style ) {
                $this->context->controller->addCSS( $style );
            }
        }

        $this->context->controller->addJS( $this->asset_media_path . 'min-js/slick.min.js' );

        $this->context->controller->addJS( $this->asset_media_path . 'min-js/modernizr.custom.min.js' );
        $this->context->controller->addJS( $this->asset_media_path . 'min-js/jquery.appear.min.js' );

        $this->context->controller->addJS( $this->asset_media_path . 'min-js/countUp.min.js' );
        $this->context->controller->addJS( $this->asset_media_path . 'min-js/count-timer.min.js' );

        if ( ! empty( self::$front_scripts ) ) {
            foreach ( self::$front_scripts as $js ) {
...
//        $this->front_js[] = array('ui.core', 'effects.core', 'ui.effect', 'ui.widget', 'ui.accordion', 'ui.progressbar', 'ui.tabs', 'ui.sortable', 'ui.draggable');
        ...
        // ultimate----------------------------
        if ( ! empty( self::$front_inline_tags ) ) {
            foreach ( self::$front_inline_tags as $tag ) {
                $content .= "{$tag}\n";
            }
        }
        // ultimate----------------------------
...
//        $this->ajaxController = '//' . $_SERVER['HTTP_HOST'] . __PS_BASE_URI__ . $this->context->controller->admin_webpath . '/' . $this->context->link->getAdminLink('VC_ajax');
        ...
        if (self::condition()) { 
            ...
            || Tools::getValue('controller')=='AdminCmsContent' 
            || Tools::getValue('controller')=='AdminCategories' 
            || Tools::getValue('controller')=='AdminManufacturers' 
            )){
                 // $this->context->controller->addJS($modulepath . 'js/jquery-1.11.0.min.js');
                 $admin_webpath = str_ireplace(_PS_CORE_DIR_, '', _PS_ADMIN_DIR_);
                 $this->addCSS(__PS_BASE_URI__ . $this->context->controller->admin_webpath . '/themes/default/public/theme.css');
                 // $this->context->controller->addJS($modulepath . 'js/jquery-migrate-1.2.1.min.js'); 
            ...
            ?>
            <script type="text/javascript">
                function lang_for_1_7_6(lang_code){
                    var js_lang_array = [];
                    <?php
                        ...
                        ?> 
                        return js_lang_array[lang_code];
                }
                function sds_id_lang(lang_code = null) { 
                    if(lang_code == null){
                        if(pageType == 'cms'){
                        var lang_code = $('#cms_page_meta_title').html();
                        lang_code = $.trim(lang_code);
                        var id_lang = window.lang_for_1_7_6(lang_code); 
                        }else if(pageType == 'category'){
                            var lang_code = $('#category_meta_title').html();
                            lang_code = $.trim(lang_code);
                            var id_lang = window.lang_for_1_7_6(lang_code); 
                        }else if(pageType == 'manufacturer'){
                            var lang_code = $('#manufacturer_meta_title').html();
                            lang_code = $.trim(lang_code);
                            var id_lang = window.lang_for_1_7_6(lang_code);
                            // alert(id_lang);
                        }else{
                ...
                function changeFrontEditorLang(lang_code = null) {  
                    var switch_front = $('a.wpb_switch-to-front-composer');
                    var found = sds_id_lang(lang_code); 
                ...
                $(function () {
                    ...
                    $(document.body).on('click', '.js-locale-item', function () {
                        var lang_code = $(this).attr("data-locale"); 
                        changeFrontEditorLang(lang_code);
                    });
                    $(document.body).on('click', '.translationsLocales a', function () {
                        var lang_code = $(this).attr("data-locale"); 
                        changeFrontEditorLang(lang_code);
                    });

                    $(document.body).on('change', '#form_switch_language', function () {
                        var lang_code = $('#form_switch_language').val();  
                        changeFrontEditorLang(lang_code);
                    }); 
    ...
    public function hookDisplayBackOfficeFooter()
    {
        ...
        if(Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')){
            ...
            $modulepath = $this->_path . 'assets/';
            echo '<script type="text/javascript" src="'.$modulepath . 'js/backend/jquery.autocomplete.js"></script>'; 
        ...
        if(Tools::getValue('controller')=='AdminCmsContent' && Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')){
            $addCustomBoostrap = true;
        }
        if(Tools::getValue('controller')=='AdminCategories' && Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')){
            $addCustomBoostrap = true;
        }
        ...
        if (self::condition()) {
            ...
            // ultimate----------------------------
            if ( Tools::getValue( 'controller' ) === 'VC_frontend' ) {
                self::$backoffice_scripts[] = $this->asset_media_path . 'js/vc-inline-editor.js';
    ...
    public static function directPreview() {
        $controller = Tools::getValue( 'controller' );
        ...
        if( Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')){
            switch ($controller) {
                case 'AdminCategories':
                    ...
                    $url = $link->getCategoryLink($id_admin);
                break;
                case 'AdminProducts':
                    $link_array = explode('products/',$actual_link);
                    $link_array = explode('/edit',$link_array[1]);  
                    $id_admin = (int)$link_array[0];
                    $url = $link->getProductLink($id_admin);
                break;
                case 'AdminManufacturers':
                    $link_array = explode('brands/',$actual_link);
                    $link_array = explode('/edit',$link_array[1]);  
                    $id_admin = (int)$link_array[0];
                    $url = $link->getManufacturerLink($id_admin);
                break;
                case 'AdminSuppliers':  
                    $id_admin = Tools::getValue('id_supplier');
                    $url = $link->getSupplierLink($id_admin);
                break;
                case 'AdminCmsContent':
                    $link_array = explode('cms-pages/',$actual_link);
                    $link_array = explode('/edit',$link_array[1]);  
                    $id_admin = (int)$link_array[0];
                    $url = $link->getCmsLink($id_admin);
                    break; 
                }  
        }else{
            switch ($controller) {
                case 'AdminCategories':  
                    $id_admin = Tools::getValue('id_category');
                    $url = $link->getCategoryLink($id_admin);
                break;
                case 'AdminProducts':
                    $link_array = explode('products/',$actual_link);
                ...
                break;
                case 'AdminManufacturers':   
                    $id_admin = Tools::getValue('id_manufacturers');
                    $url = $link->getManufacturerLink($id_admin);
                break;
                case 'AdminSuppliers':  
                    $id_admin = Tools::getValue('id_supplier');
                ...
                break;
                case 'AdminCmsContent': 
                    $id_admin = Tools::getValue('id_cms');
                    $url = $link->getCmsLink($id_admin);
                    break; 
                }  
        }
    ...
    public function hookDisplayFooter1($params)
    {
        $rsts = $this->contenthookvalue('displayFooter1');
        return $rsts;
    }

    public function hookDisplayFooter2($params)
    {
        $rsts = $this->contenthookvalue('displayFooter2');
        return $rsts;
    }

    public function hookDisplayFooter3($params)
    {
        $rsts = $this->contenthookvalue('displayFooter3');
        return $rsts;
    }

    public function hookDisplayFooter4($params)
    {
        $rsts = $this->contenthookvalue('displayFooter4');
        return $rsts;
    }

    public function hookDisplayFooter5($params)
    {
        $rsts = $this->contenthookvalue('displayFooter5');
        return $rsts;
    }

    public function hookDisplayFooter6($params)
    {
        $rsts = $this->contenthookvalue('displayFooter6');
        return $rsts;
    }

    public function hookDisplayFooter7($params)
    {
        $rsts = $this->contenthookvalue('displayFooter7');
        return $rsts;
    }

    public function hookDisplayFooter8($params)
    {
        $rsts = $this->contenthookvalue('displayFooter8');
        return $rsts;
    }

    public function hookDisplayFooter9($params)
    {
        $rsts = $this->contenthookvalue('displayFooter9');
        return $rsts;
    }

    public function hookDisplayFooter10($params)
    {
        $rsts = $this->contenthookvalue('displayFooter10');
        return $rsts;
    }

    public function hookDisplayFooter11($params)
    {
        $rsts = $this->contenthookvalue('displayFooter11');
        return $rsts;
    }

    public function hookDisplayFooter12($params)
    {
        $rsts = $this->contenthookvalue('displayFooter12');
        return $rsts;
    }

    public function hookDisplayFooter13($params)
    {
        $rsts = $this->contenthookvalue('displayFooter13');
        return $rsts;
    }

    public function hookDisplayFooter14($params)
    {
        $rsts = $this->contenthookvalue('displayFooter14');
        return $rsts;
    }

    public function hookDisplayFooter15($params)
    {
        $rsts = $this->contenthookvalue('displayFooter15');
        return $rsts;
    }

    public function hookDisplayFooter16($params)
    {
        $rsts = $this->contenthookvalue('displayFooter16');
        return $rsts;
    }

    public function hookDisplayFooter17($params)
    {
        $rsts = $this->contenthookvalue('displayFooter17');
        return $rsts;
    }

    public function hookDisplayFooter18($params)
    {
        $rsts = $this->contenthookvalue('displayFooter18');
        return $rsts;
    }

    public function hookDisplayFooter19($params)
    {
        $rsts = $this->contenthookvalue('displayFooter19');
        return $rsts;
    }

    public function hookDisplayFooter20($params)
    {
        $rsts = $this->contenthookvalue('displayFooter20');
        return $rsts;
    }

    public function hookDisplayFooter21($params)
    {
        $rsts = $this->contenthookvalue('displayFooter21');
        return $rsts;
    }

    public function hookDisplayFooter22($params)
    {
        $rsts = $this->contenthookvalue('displayFooter22');
        return $rsts;
    }

    public function hookDisplayFooter23($params)
    {
        $rsts = $this->contenthookvalue('displayFooter23');
        return $rsts;
    }

    public function hookDisplayFooter24($params)
    {
        $rsts = $this->contenthookvalue('displayFooter24');
        return $rsts;
    }

    public function hookDisplayFooter25($params)
    {
        $rsts = $this->contenthookvalue('displayFooter25');
        return $rsts;
    }

    public function hookDisplayFooter26($params)
    {
        $rsts = $this->contenthookvalue('displayFooter26');
        return $rsts;
    }

    public function hookDisplayFooter27($params)
    {
        $rsts = $this->contenthookvalue('displayFooter27');
        return $rsts;
    }

    public function hookDisplayFooter28($params)
    {
        $rsts = $this->contenthookvalue('displayFooter28');
        return $rsts;
    }

    public function hookDisplayFooter29($params)
    {
        $rsts = $this->contenthookvalue('displayFooter29');
        return $rsts;
    }

    public function hookDisplayFooter30($params)
    {
        $rsts = $this->contenthookvalue('displayFooter30');
        return $rsts;
    }

    public function hookDisplayFooter31($params)
    {
        $rsts = $this->contenthookvalue('displayFooter31');
        return $rsts;
    }

    public function hookDisplayFooter32($params)
    {
        $rsts = $this->contenthookvalue('displayFooter32');
        return $rsts;
    }

    public function hookDisplayFooter33($params)
    {
        $rsts = $this->contenthookvalue('displayFooter33');
        return $rsts;
    }

    public function hookDisplayFooter34($params)
    {
        $rsts = $this->contenthookvalue('displayFooter34');
        return $rsts;
    }

    public function hookDisplayFooter35($params)
    {
        $rsts = $this->contenthookvalue('displayFooter35');
        return $rsts;
    }
