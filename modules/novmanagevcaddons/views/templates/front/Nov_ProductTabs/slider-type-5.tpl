...
<div class="nov-producttabs tab-slider tab-style-{$tabs_style}{if $show_arrows == 'true'} show_arrows{/if} {$el_class}">
	<div class="block-producttabs clearfix">
		{if $tabs_style == '2' || $tabs_style == '3' || $tabs_style == '4' || $tabs_style == '5' || $tabs_style == '7'}
			...
			{/if}
		{/if}
		<div class="d-flex align-items-center d-xs-block{if $tabs_style == '2' || $tabs_style == '3' || $tabs_style == '4' || $tabs_style == '4' || $tabs_style == '7'} justify-content-center{/if} block_nav">
			...
			<ul class="nav nav-tabs" role="tablist">
				{foreach from=$tabs item=tab_item name=tab}
					{if $tab_item == 'newproducts'}
		      			<li class="nav-item">
		      				<a href="#newproducts{$tab}" class="nav-link active" role="tab" data-toggle="tab">{l s='New Arrivals' mod='novmanagevcaddons'}</a>
		      			</li>
					{/if}
					{if $tab_item == 'bestsellers'}
		      			<li class="nav-item">
		      				<a href="#bestsellers{$tab}" class="nav-link" role="tab" data-toggle="tab">{l s='Bestseller' mod='novmanagevcaddons'}
		      				</a>
		      			</li>
					{/if}
					{if $tab_item == 'specialproducts'}
		      			<li class="nav-item">
		      				<a href="#specialproducts{$tab}" class="nav-link" role="tab" data-toggle="tab">{l s='On Sale' mod='novmanagevcaddons'}</a></li>
					{/if}
					{if $tab_item == 'featureproducts'}
		      			<li class="nav-item">
		      				<a href="#featureproducts{$tab}" class="nav-link" role="tab" data-toggle="tab" >{l s='Featured Product' mod='novmanagevcaddons'}</a></li>
					{/if}
			  	{/foreach}
		    </ul>
		</div>
