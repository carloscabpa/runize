...
<div class="nov-subscription text-center style-4{if isset($el_class) && !empty($el_class)}{$el_class}{/if}" style="background: {$accent_color};">
	<div class="block block-subscription clearfix">
		{if isset($title) && !empty($title)}
			<div class="img_email mb-25">{$subs_icon nofilter}</div>
			<div class="title_block mb-10">{$title}</div>
			{if isset($sub_title) && !empty($sub_title)}
				<span class="sub_title mb-25">{$sub_title}</span>
			{/if}
